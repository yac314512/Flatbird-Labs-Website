...
function serveFile(reqPath, res) {
  let cleanPath = reqPath === "/" ? "/index.html" : reqPath;

  // Pretty URLs: if no extension and the .html file exists, serve that.
  if (!path.extname(cleanPath)) {
    const htmlCandidate = path.join(PUBLIC_DIR, `${cleanPath.replace(/\/$/, "")}.html`);
    if (fs.existsSync(htmlCandidate) && !fs.statSync(htmlCandidate).isDirectory()) {
